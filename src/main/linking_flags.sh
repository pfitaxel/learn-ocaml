...
case $(uname -s) in
    Linux)
        case $(. /etc/os-release && echo $ID) in
            alpine)
                COMMON_LIBS="safepass_stubs netsys rt cryptokit_stubs z zarith gmp ssl_stubs ssl crypto cstruct_stubs bigstringaf_stubs lwt_unix_stubs unix c"
        ...
        ;;
    Darwin)
        COMMON_LIBS="safepass_stubs netsys cryptokit_stubs z zarith gmp ssl_stubs /usr/local/opt/openssl/lib/libssl.a /usr/local/opt/openssl/lib/libcrypto.a cstruct_stubs bigstringaf_stubs lwt_unix_stubs bigarray unix"
